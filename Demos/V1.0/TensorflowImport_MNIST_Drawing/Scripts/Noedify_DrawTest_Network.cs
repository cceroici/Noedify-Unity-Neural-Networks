...
public class Noedify_DrawTest_Network : MonoBehaviour
{
    ...
    Noedify.Net BuildAndImportModel()
    {
        ...
        if (importTensorflowModel)
        {
            ...
            status = Noedify_Manager.ImportNetworkParameters(net, "FC_mnist_600x300x140_parameters");
            if (status)
    ...
    void TestMNISTDrawing(Noedify.Net net)
    {
        ...
        float[,,] modelInputsFormatted = new float[1, 28, 28];
        for (int i = 0; i < 28; i++)
            for (int j=0; j < 28; j++)
                modelInputsFormatted[0, i, j] = modelInputs[i];

        System.Diagnostics.Stopwatch sw = new System.Diagnostics.Stopwatch();
        sw.Start();
        solver.Evaluate(net, new Noedify.InputArray(modelInputsFormatted), Noedify_Solver.SolverMethod.CPU);
        sw.Stop();
